...
Pure Python WebSocket Implementation - High Performance & Lightweight
Optimized for Stryker Bangladesh
"""
...
from config import (
    OPENAI_API_KEY, DB_PATH, COLLECTION_NAME, 
    EMBEDDING_MODEL, CHAT_MODEL, MAX_CONTEXT_CHUNKS,
    TIMEOUT_SECONDS, MAX_CONVERSATION_HISTORY,
...
async def handle_client(websocket: WebSocketServerProtocol, path: str):
    ...
    # Extract client_id from path (e.g., /ws/client123)
    client_id = path.strip('/').split('/')[-1] if '/' in path else f"client_{id(websocket)}"
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("Stryker AI ChatBot WebSocket Server")
    logger.info("=" * 70)
    logger.info(f"Host: {SERVER_HOST}")
    logger.info(f"Port: {SERVER_PORT}")
    logger.info(f"WebSocket URL: ws://{SERVER_HOST}:{SERVER_PORT}/ws/{{client_id}}")
    logger.info("=" * 70)
